...
def generate_segment_campaigns(rfm_df):
    campaigns = []
    recency_q = rfm_df['Recency'].quantile([0.33, 0.66]).values
    ...
    for idx, row in rfm_df.iterrows():
        ...
        recency = row['Recency']; freq = row['Frequency']; mon = row['Monetary']
        if (recency <= recency_q[0]) and (freq >= freq_q[1]) and (mon >= mon_q[1]):
            campaigns.append({
                'customer': seg_label,
                'segment': 'VIP',
                'suggestion': 'Invite to VIP program; offer 15% off on premium products; early access to new offers.'
            })
        elif (recency > recency_q[1]) and (freq <= freq_q[0]):
            campaigns.append({
                'customer': seg_label,
                'segment': 'Churn-risk',
...
if {'Customer type','Gender','Product line'}.issubset(df.columns):
    top_by_group = (df.groupby(['Customer type','Gender','Product line'])[monetary_col]
                    .sum().reset_index()
                    .sort_values(['Customer type','Gender', monetary_col], ascending=[True,True,False]))
    bundles = []
    for (ctype, g), group_df in top_by_group.groupby(['Customer type','Gender']):
        ...
        if len(top_lines) >= 2:
            bundles.append({
                ...
                'suggestion': f"Offer 10% off when buying {top_lines[0]} with {top_lines[1]} for {ctype} {g} customers."
            })
...
if store_attr is not None:
    ...
    for _, row in sa.iterrows():
        if row['sales_per_branch'] < median_sales:
            actions.append({
                'branch': row['Branch'],
                'action': 'Underperforming: run local promotions, adjust stock for high-margin items.'
            })
        else:
            actions.append({
                'branch': row['Branch'],
                'action': 'Performing well: maintain inventory, test premium product launches and loyalty incentives.'
            })
...
all_reco = {
    ...
    'bundles': bundles_df if 'bundles_df' in locals() else pd.DataFrame(),
    'store_actions': actions_df if 'actions_df' in locals() else pd.DataFrame()
}
...
def combine_recos_to_csv(dict_of_dfs):
    ...
    for k,v in dict_of_dfs.items():
        if v.shape[0] > 0:
            out.append(pd.DataFrame({'section': k}))
            out.append(v)
    if out:
