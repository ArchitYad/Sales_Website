...
def show_analysis():
    ...
    # -------------------------
    # 2️⃣ Shopping Behavior & Decision Tree
    # -------------------------
    ...
    required_cols = {'Customer type','Gender','Product line','Unit price','Quantity'}
    ...
    if "Branch" in df.columns:
        ...
        # Folium map with branch markers
        if "City" in df.columns:
            ...
            for city, coords in city_coords.items():
                ...
                folium.CircleMarker(
                    location=coords,
                    radius=max(3, total_sales / 5000),
                    popup=f"{city}: {total_sales:.2f}",
                    color="blue",
                    fill=True
                ).add_to(m)
